...
function createGame(pacmanSelector, mazeSelector) {
    ...
    function gameLoop() {
        ...
        if (areColliding(pacman, ball)) {
            ctxMaze.clearRect(ball.x, ball.y - 2, ball.size + 2, ball.size + 4);
            balls.splice(index, 1);
        }
        });

        var isPacmanCollindingWithWall = false,
            futurePosition = {
        ...
        walls.forEach(function (wall){
            if (areColliding(futurePosition, wall)) {
             isPacmanCollindingWithWall = true;   
            }
            });

        if (!isPacmanCollindingWithWall) {
    ...
    return {
        start: function() {
            [balls, walls] = drawMazeAndGetBallsAndWalls(ctxMaze, maze, pacman.size + 2);
            gameLoop();
        }
    };
}